...
export function compiler(global:GlobalConfig,project:ProjectConfig){
    let output=process.cwd()+'/'+project.output+'.sbin'
    //寻找目录下所有.sl文件
    const file: string[] = []
...
export async function init(){
    ...
    const output=await input({message:'输出路径:',default:`${folder_name}.sbin`})
